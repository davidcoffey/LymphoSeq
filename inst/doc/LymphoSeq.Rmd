...
output: 
    html_document:
        ...
        df_print: paged
vignette: >
    %\VignetteIndexEntry{An introduction to LymphoSeq}
...
Notice that each data frame listed in the `TCRB.list` object is named according the ImmunoSEQ file name files.  If different names are desired, you may rename the original `.tsv` files or assign `names(TCRB.list)` to a new character vector of desired names in the same order as the list.
...
In LymphoSeq, you can perform a multiple sequence alignment using one of three methods provided by the Bioconductor msa package (ClustalW, ClustalOmega, or Muscle) and output results to the consule or as a pdf file. One may perform the alignment of all amino acid or nucleotide sequences in a single sample.
