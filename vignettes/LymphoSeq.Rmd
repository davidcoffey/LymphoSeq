...
Notice that each data frame listed in the `TCRB.list` object is named according the ImmunoSEQ file name files.  If different names are desired, you may rename the original `.tsv` files or assign `names(TCRB.list)` to a new character vector of desitred names within R.
...
```{r, results = "hide"}
productive.aa <- productiveSeq(file.list = TCRB.list, aggregate = "aminoAcid", 
                               prevalence = FALSE)
```
...
```{r, results = "hide"}
productive.nt <- productiveSeq(file.list = TCRB.list, aggregate = "nucleotide", 
                               prevalence = FALSE)
```
...
```{r, comment = ""}
head(productive.aa[["TRB_Unsorted_0"]])
```

Finally, notice that the productive.nt data frame for sample "TRB_Unsorted_949" below has additional columns not present in productive.aa but are in TCRB.list.  This is because the data frame was aggregated by nucleotide sequence and all of the original columns from TCRB.list were carried over.

```{r, comment = ""}
head(productive.nt[["TRB_Unsorted_0"]])
```
