...
getTCRMatrix <- function(tra_path, trb_path, subsets=100) {
    ...
    ks.tra.samplename <- tools::file_path_sans_ext(basename(ks.tra.filenames)) %>% str_split("_TRA", n = 2, simplify = TRUE) %>% as_tibble(.name_repair = ~ c("V1", "V2")) %>% select(V1)
    ks.trb.samplename <- tools::file_path_sans_ext(basename(ks.trb.filenames)) %>% str_split("_TRB", n = 2, simplify = TRUE) %>% as_tibble(.name_repair = ~ c("V1", "V2")) %>% select(V1)
    ...
    ks.tra.filenames.matched <- ks.tra.filenames[ks.tra.trb.matched$V1][1:subsets]
    ks.trb.filenames.matched <- ks.trb.filenames[ks.tra.trb.matched$V1][1:subsets]
...
ks_tcr_list <- getTCRMatrix("/Volumes/home/fngs/ImmunoSEQ/TRA/KS", "/Volumes/home/fngs/ImmunoSEQ/TRB/KS")
ks_tra_mtx <- ks_tcr_list[[1]]
ks_trb_mtx <- ks_tcr_list[[2]]
ks_tra_list <- ks_tcr_list[[3]]
ks_trb_list <- ks_tcr_list[[4]]

ks_paired <- ks_tra_mtx %*% t(ks_trb_mtx)
...
clonotypes <- list.files("~/fuser/kstme/clonotypes", pattern = ".csv", full.names = TRUE) %>% map(readClonotypes) %>% reduce(rbind)
...
clonotypes_paired <- clonotypes_paired %>% select(sample.x, aminoAcid.x, aminoAcid.y, frequency.x, frequency.y) %>% rename(tra_sequence = "aminoAcid.x", trb_sequence = "aminoAcid.y", tra_count = "frequency.x", trb_count = "frequency.y")

adaptive_v_10x <- inner_join(clonotypes_paired, ks_paired , by=c("tra_sequence", "trb_sequence"))
